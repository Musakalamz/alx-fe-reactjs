...
export async function searchUsers({
  ...
  if (!q)
    return { data: { users: [], totalCount: 0 }, error: "validation_error" };
  try {
    ...
    return {
      data: { users, totalCount: res?.data?.total_count ?? users.length },
    };
  } catch (err) {
    const status = err?.response?.status;
    if (status === 404)
      return { data: { users: [], totalCount: 0 }, error: "not_found" };
    if (status === 403)
      return { data: { users: [], totalCount: 0 }, error: "rate_limited" };
    if (status === 422)
      return { data: { users: [], totalCount: 0 }, error: "validation_error" };
